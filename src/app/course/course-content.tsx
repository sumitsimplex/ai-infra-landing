...
export default function CourseContent() {
  ...
  return (
    <div className="h-screen flex flex-col bg-zinc-950 text-white overflow-hidden">
      ...
      {/* Body — sidebar + content, both scroll independently */}
      <div className="flex flex-1 overflow-hidden">
        ...
        <AnimatePresence initial={false}>
          {sidebarOpen && (
            ...
            >
              <div className="w-80 h-full overflow-y-auto p-4 scrollbar-thin">
                {weeks.map((week) => {
              ...
              <article className="course-content">
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    ol: ({ children }) => (
                      <ol className="space-y-2 mb-5 ml-1 list-decimal list-inside">
                        {children}
                      </ol>
                    ),
                    li: ({ children }) => (
                      <li className="text-zinc-300 leading-relaxed flex items-start gap-2">
                        <span className="text-indigo-400 mt-1.5 flex-shrink-0 text-xs">
                          *
                        </span>
                        <span>{children}</span>
                      </li>
                    ...
                    code: ({ className, children }) => {
                      ...
                      return (
                        <code className="text-indigo-300 bg-zinc-800/80 px-1.5 py-0.5 rounded text-sm font-mono">
                          {children}
                        </code>
                      );
                    },
                    pre: ({ children }) => (
                      <pre className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-5 overflow-x-auto text-sm leading-relaxed font-mono">
                        {children}
                      </pre>
                    ),
                    table: ({ children }) => (
                    ...
                    blockquote: ({ children }) => (
                      <blockquote className="border-l-2 border-indigo-500 pl-4 py-1 mb-5 text-zinc-400 italic bg-indigo-500/5 rounded-r-lg pr-4">
                        {children}
                      </blockquote>
                    ),
                    hr: () => (
                    ...
                    input: ({ checked }) => (
                      <span
                        className={`inline-block w-4 h-4 rounded border mr-2 flex-shrink-0 mt-0.5 ${
                          checked
                  ...
                  {nextModule && (
                    <button
                      onClick={() => {
                        ...
                        // Expand the week containing the next module
